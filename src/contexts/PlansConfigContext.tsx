...
export const PlansConfigProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchConfigs = useCallback(async () => {
    try {
      const { data, error } = await supabase
        ...
        .select("plan_type, volume, price_inr, store_limit, product_limit, customer_limit");
      if (error) {
  ...
  return (
    <PlansConfigContext.Provider value={{ configs, loading, getPriceINR, getPlanLimits, refetch: fetchConfigs }}>
      {children}
