import { getPublicOrigin } from "./publicUrl";

// Browser-safe MD5 for deterministic invoice token (matches Postgres md5())
// Source: tiny implementation (public domain). Not used for cryptographic security —
// just a stable hash so the same token can be verified server-side.
...
export const getInvoiceToken = (orderId: string): string =>
  md5(orderId + "evixpos-invoice-v1").slice(0, 16);

export const buildInvoiceUrl = (orderId: string): string =>
  `${getPublicOrigin()}/i/${orderId}?t=${getInvoiceToken(orderId)}`;
