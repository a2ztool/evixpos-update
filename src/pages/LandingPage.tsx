  ...
  <div className="overflow-hidden relative">
    ...
    >
      {[...items, ...items].map((item, i) => (
        <span
          key={i}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-card border border-border/50 text-sm font-medium text-muted-foreground shrink-0"
        >
        ...
        {featureCards.map((i, idx) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`relative z-10 w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 ${
                  isActive
              ...
              <div className="relative z-10 flex-1 min-w-0">
                <div
                  className={`text-[13px] font-semibold truncate transition-colors duration-200 ${isActive ? "text-foreground" : "text-muted-foreground group-hover:text-foreground"}`}
                >
                  {get(`feature_${i}_title`) || `Feature ${i}`}
                </div>
                <div
                  className={`text-[11px] truncate transition-colors duration-200 ${isActive ? "text-primary" : "text-muted-foreground/60"}`}
                >
                  {get(`feature_${i}_badge`, `Feature`)}
                </div>
              </div>
              <span
                className={`relative z-10 text-[10px] font-bold tabular-nums transition-colors ${isActive ? "text-primary" : "text-muted-foreground/30"}`}
              >
...
const LandingPage = () => {
  ...
  const {
    getPriceINR: dynamicGetPriceINR,
    getPriceBDT: dynamicGetPriceBDT,
    getPlanLimits: dynamicGetPlanLimits,
  } = usePlansConfig();
  ...
  return (
    <div className="min-h-[100dvh] md:min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      >
        ...
        {bannerActive && (
          <div
            className={`bg-gradient-to-r from-primary via-primary/90 to-primary/80 text-primary-foreground text-center text-[10px] sm:text-sm font-medium relative overflow-hidden transition-all duration-300 ${scrolled ? "max-h-0 opacity-0" : "h-10 sm:h-14 opacity-100"}`}
          >
            ...
            <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-full flex items-center justify-center gap-1.5 sm:gap-3 relative leading-none">
              ...
              <span className="line-clamp-1 leading-none">
                {get(
                  "banner_subtitle",
                  "Now with Offline POS + Online Store in one panel — Get 50% off your first 3 months!",
                )}
              </span>
              <Button
                size="sm"
          ...
          <motion.nav
            ...
            className={`max-w-5xl mx-auto flex items-center justify-between px-3 sm:px-6 lg:px-7 transition-all duration-500 ease-out ${
              scrolled
                ...
                : "h-14 sm:h-16 rounded-[18px] sm:rounded-[22px] bg-card/60 backdrop-blur-xl border border-border/30 shadow-[0_12px_40px_-6px_hsl(var(--foreground)/0.1)]"
            }`}
          >
            <div className="flex items-center gap-2 shrink-0">
              <img
                src={get("brand_logo") || brandLogo}
                alt={get("brand_name", "EvixPOS")}
                width={140}
                height={32}
                loading="eager"
                decoding="async"
                fetchPriority="high"
                className={`w-auto transition-all duration-300 ${scrolled ? "h-6 sm:h-7" : "h-7 sm:h-8"}`}
              />
            </div>
            <div className="hidden lg:flex items-center gap-0.5 absolute left-1/2 -translate-x-1/2">
              {DEFAULT_NAV_LINKS.map((link) => (
                <button
                  ...
                  className="relative px-4 py-2 rounded-xl text-[13px] font-medium text-muted-foreground hover:text-foreground transition-all duration-200 group"
                >
            ...
            <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
              ...
              <Button
                size="sm"
                onClick={() => navigate("/auth")}
                className="gap-1 sm:gap-1.5 text-[11px] sm:text-[13px] font-semibold !h-7 sm:!h-10 sm:px-5 rounded-full bg-primary hover:bg-primary/90 shadow-[0_4px_16px_-4px_hsl(var(--primary)/0.4)] hover:shadow-[0_6px_20px_-4px_hsl(var(--primary)/0.5)] active:scale-[0.96] transition-all duration-200 py-0 px-2.5 leading-none min-h-0"
              >
              ...
              >
                <div className="flex flex-col gap-1">
                  <span
                    className={`w-4 h-0.5 bg-foreground rounded-full transition-all duration-300 ${mobileMenuOpen ? "rotate-45 translate-y-1.5" : ""}`}
                  />
                  <span
                    className={`w-4 h-0.5 bg-foreground rounded-full transition-all duration-300 ${mobileMenuOpen ? "opacity-0" : ""}`}
                  />
                  <span
                    className={`w-4 h-0.5 bg-foreground rounded-full transition-all duration-300 ${mobileMenuOpen ? "-rotate-45 -translate-y-1.5" : ""}`}
                  />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            <div className="relative rounded-full p-[1.5px] max-w-[95vw] overflow-hidden">
              ...
              <div className="relative inline-flex items-center gap-1.5 sm:gap-3 px-2.5 sm:px-5 rounded-full bg-card shadow-sm h-7 sm:h-8">
                  ...
                  {[1, 2, 3].map((i) => {
                    ...
                    return img ? (
                      <img
                        key={i}
                        src={img}
                        className="w-4 h-4 sm:w-6 sm:h-6 rounded-full border-2 border-card object-cover"
                        alt=""
                      />
                    ) : (
                      <div
                        key={i}
                        className="w-4 h-4 sm:w-6 sm:h-6 rounded-full border-2 border-card bg-gradient-to-br from-primary/40 to-primary/20 flex items-center justify-center"
                      >
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => navigate("/auth")}
                  className="!h-5 sm:!h-6 px-1.5 sm:px-2.5 text-[10px] sm:text-xs font-semibold text-primary hover:text-primary shrink-0 leading-none min-h-0"
                >
                  {get("hero_social_proof_cta", "Join")}{" "}
          ...
          >
            ...
            >
              {get(
                "hero_subtitle",
                "Manage your online store, POS billing, walk-in sales, inventory, customers & profit analytics — all from one powerful panel. No more juggling separate tools.",
              )}
            </motion.p>
            ...
            <div className="flex items-center justify-center gap-5 text-sm text-muted-foreground flex-wrap">
              <span className="flex items-center gap-1.5">
                ...
                {get("hero_bullet_1", "Online + Offline in one panel")}
              </span>
              <span className="flex items-center gap-1.5">
                <CheckCircle2 className="h-4 w-4 text-primary" /> {get("hero_bullet_2", "No credit card needed")}
              </span>
              <span className="flex items-center gap-1.5">
                <CheckCircle2 className="h-4 w-4 text-primary" /> {get("hero_bullet_3", "Setup in 2 minutes")}
              </span>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="relative rounded-2xl overflow-hidden border border-border/50 shadow-2xl shadow-primary/10">
              <div className="bg-muted/80 h-9 flex items-center gap-2 px-4 border-b border-border/50">
                ...
                <div className="flex-1 flex justify-center">
                  <div className="bg-background/60 rounded-md px-8 py-0.5 text-[10px] text-muted-foreground">
                    {get("hero_browser_url", "app.evixpos.com")}
                  </div>
                </div>
              </div>
              <img
                src={get("hero_image") || dashboardPreview}
                alt={get("hero_image_alt", "EvixPOS Dashboard")}
                className="w-full"
                width={1280}
                height={800}
              />
            ...
            <motion.div
              ...
              className="absolute -bottom-5 -left-4 sm:-bottom-6 sm:-left-8 bg-card rounded-xl border border-border/50 p-4 shadow-xl backdrop-blur-sm"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-muted-foreground">{get("hero_card_revenue_label", "Revenue Today")}</div>
                  <div className="text-lg font-bold">{get("hero_card_revenue_value", "৳42,580")}</div>
                </div>
              </div>
            </motion.div>
            <motion.div
              ...
              className="absolute -top-3 -right-3 sm:-top-5 sm:-right-6 bg-card rounded-xl border border-border/50 p-3 shadow-xl backdrop-blur-sm"
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-[10px] text-muted-foreground">{get("hero_card_orders_label", "New Orders")}</div>
                  <div className="text-sm font-bold text-primary">{get("hero_card_orders_value", "+24")}</div>
                </div>
              </div>
            </motion.div>
          </motion.div>
      ...
      {/* ═══════════════════ TRUST STATS ═══════════════════ */}
      {show("trust") && (
        <section className="py-6 sm:py-10 border-y border-border/40 bg-muted/20 relative">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-12 max-w-sm sm:max-w-none mx-auto">
              {[
                {
                  val: get("stats_users", "3,000+"),
                  label: get("stats_users_label", "Active Businesses"),
                  icon: Users,
                },
                {
                  val: get("stats_stores", "1,983+"),
                  label: get("stats_stores_label", "Online & Offline Stores"),
                  icon: Store,
                },
                {
                  val: get("stats_orders", "50K+"),
                  label: get("stats_orders_label", "Orders Processed"),
                  icon: Package,
                },
                ...
                <AnimItem
                  key={s.label}
                  delay={idx * 0.1}
                  className="flex sm:block items-center gap-3 sm:text-center group py-2 sm:py-0 px-3 sm:px-0 rounded-xl sm:rounded-none bg-card/40 sm:bg-transparent border sm:border-0 border-border/30"
                >
      ...
      {show("pain_points") && (
        <section id="pain-points" className="py-10 sm:py-14 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <AnimSection className="text-center max-w-3xl mx-auto mb-10 sm:mb-12">
              <Badge
                variant="outline"
                className="mb-5 text-destructive border-destructive/30 bg-destructive/5 backdrop-blur-sm px-3.5 py-1.5 rounded-full font-medium"
              >
              ...
              <p className="text-muted-foreground text-base sm:text-lg leading-relaxed">
                {get(
                  "pain_subtitle",
                  "You sell online and from a physical counter — but you're using separate tools for each. Disconnected data, missed sales, and zero visibility into real profits.",
                )}
              </p>
            </AnimSection>
            ...
            <div className="relative grid lg:grid-cols-2 gap-6 lg:gap-8">
              ...
              <AnimItem>
                <div className="group relative rounded-3xl h-full">
                  {/* gradient border */}
                  <div
                    className="absolute -inset-px rounded-3xl bg-gradient-to-b from-destructive/30 via-destructive/10 to-transparent opacity-70"
                    aria-hidden
                  />
                  <div className="relative rounded-3xl bg-card/80 backdrop-blur-xl overflow-hidden h-full border border-destructive/10">
                    ...
                    <div className="relative flex items-center justify-between px-7 py-5 border-b border-destructive/10">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-[10px] uppercase tracking-[0.18em] text-destructive/70 font-semibold">
                            Before
                          </div>
                          <span className="font-bold text-foreground text-base leading-tight">
                            {get("pain_old_title", "Without EvixPOS")}
                          </span>
                        </div>
                      </div>
                    ...
                    {get("pain_old_image") && (
                      <div className="px-7 pt-5">
                        <img
                          src={get("pain_old_image")}
                          alt="Old way"
                          className="w-full rounded-2xl border border-destructive/10"
                          loading="lazy"
                        />
                      </div>
                    )}
                    ...
                    <div className="p-7 grid grid-cols-2 gap-3">
                      {[
                        ...
                        {
                          label: get("pain_metric_3_label", "Profit Visibility"),
                          val: get("pain_metric_3_val", "Guesswork"),
                        },
                        {
                          label: get("pain_metric_4_label", "Time Wasted"),
                          val: get("pain_metric_4_val", "5+ Hrs/Day"),
                        },
                      ].map((m, i) => (
                        <div
                          key={i}
                          className="relative rounded-2xl p-4 bg-gradient-to-br from-destructive/[0.06] to-transparent border border-destructive/10 hover:border-destructive/25 transition-colors"
                        >
              ...
              <AnimItem delay={0.15}>
                <div className="group relative rounded-3xl h-full">
                  {/* premium glow */}
                  <div
                    className="absolute -inset-px rounded-3xl bg-gradient-to-b from-primary/40 via-primary/15 to-transparent opacity-80"
                    aria-hidden
                  />
                  <div
                    className="absolute -inset-4 rounded-[2rem] bg-primary/10 blur-2xl opacity-50 -z-10"
                    aria-hidden
                  />
                  <div className="relative rounded-3xl bg-card/80 backdrop-blur-xl overflow-hidden h-full border border-primary/15 shadow-xl shadow-primary/5">
                    ...
                    <div className="relative flex items-center justify-between px-7 py-5 border-b border-primary/10">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="font-bold text-foreground text-base leading-tight">
                            {get("pain_new_title", "With EvixPOS")}
                          </span>
                        </div>
                      </div>
                      <Badge className="bg-primary/10 text-primary border border-primary/20 hover:bg-primary/15 rounded-full text-[10px] font-bold tracking-wider px-2.5 py-0.5 uppercase">
                        Recommended
                      </Badge>
                    </div>
                    ...
                    <div className="p-7 grid grid-cols-2 gap-3">
                      {[
                        {
                          label: get("solution_metric_1_label", "One Panel"),
                          val: get("solution_metric_1_val", "Online + Offline"),
                        },
                        {
                          label: get("solution_metric_2_label", "Data Sync"),
                          val: get("solution_metric_2_val", "Real-Time"),
                        },
                        {
                          label: get("solution_metric_3_label", "Profit View"),
                          val: get("solution_metric_3_val", "Instant"),
                        },
                        {
                          label: get("solution_metric_4_label", "Time Saved"),
                          val: get("solution_metric_4_val", "4+ Hrs/Day"),
                        },
                      ].map((m, i) => (
                        <div
                          key={i}
                          className="relative rounded-2xl p-4 bg-gradient-to-br from-primary/[0.08] to-transparent border border-primary/15 hover:border-primary/30 transition-colors"
                        >
                          <div className="text-[11px] text-muted-foreground/80 mb-1.5 font-medium">{m.label}</div>
            ...
            <AnimSection className="text-center mt-12" delay={0.2}>
              <Button
                size="lg"
                onClick={() => navigate("/auth")}
                className="gap-2 rounded-full shadow-2xl shadow-primary/30 px-8 h-12 bg-gradient-to-b from-primary to-primary/90 hover:from-primary hover:to-primary font-semibold"
              >
      ...
      {show("features") &&
        (() => {
          const featureCards = Array.from({ length: 20 }, (_, i) => i + 1).filter(
            (i) => get(`feature_${i}_title`) || get(`feature_${i}_image`),
          );
          return (
              ...
              <div className="pt-10 sm:pt-14 pb-6 sm:pb-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  <AnimSection className="text-center max-w-3xl mx-auto">
                    <Badge
                      variant="outline"
                      className="mb-5 text-primary border-primary/30 px-4 py-1.5 text-sm font-medium backdrop-blur-sm bg-card/50"
                    >
                    ...
                    <p className="text-muted-foreground text-base sm:text-lg leading-relaxed max-w-2xl mx-auto">
                      {get(
                        "features_subtitle",
                        "From e-commerce orders to walk-in POS billing — one platform handles it all.",
                      )}
                    </p>
                  </AnimSection>
                </div>
      ...
      {show("who") && (
        <section className="py-10 sm:py-14 bg-muted/20 relative overflow-hidden">
          ...
          <div className="space-y-4">
            <Marquee
              items={get(
                "who_list_1",
                "OTT Subscription|Educational Courses|Memberships|Gym & Fitness|Hosting Services|Themes & Plugins",
              ).split("|")}
            />
            <Marquee
              items={get(
                "who_list_2",
                "License Keys|Physical Products|Personal Services|SaaS Products|Logistic Services|Digital Products",
              ).split("|")}
              reverse
            />
          </div>
      ...
      {show("how_it_works") && (
        <section id="how-it-works" className="py-10 sm:py-14 bg-muted/20 relative overflow-hidden">
            ...
            <AnimSection className="text-center max-w-3xl mx-auto mb-10">
              ...
              <p className="text-muted-foreground text-lg leading-relaxed">
                {get(
                  ...
                  "Whether you're setting up an online store or a physical POS — you'll be selling in four simple steps.",
                )}
              </p>
            </AnimSection>
      ...
      {show("about") && (
        <section id="about" className="py-10 sm:py-14 relative overflow-hidden">
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              <AnimSection>
                ...
                <p className="text-muted-foreground text-lg leading-relaxed mb-6">
                  {get(
                    "about_desc_1",
                    "EvixPOS is the all-in-one business management platform that bridges the gap between online and offline commerce. Whether you run an e-commerce store, a physical retail shop, or both — we give you one unified panel to manage everything.",
                  )}
                </p>
                <p className="text-muted-foreground leading-relaxed mb-8">
                  {get(
                    "about_desc_2",
                    "Founded with the mission to eliminate the chaos of juggling separate tools. We believe every business deserves enterprise-grade management without enterprise-grade complexity.",
                  )}
                </p>
              ...
              <AnimSection delay={0.2}>
                <div className="relative">
                  ...
                  <div className="relative rounded-2xl overflow-hidden border border-border/50 shadow-2xl">
                    <img
                      src={get("about_image") || dashboardPreview}
                      alt="About EvixPOS"
                      className="w-full"
                      loading="lazy"
                    />
      ...
      {show("why_us") && (
        <section id="why-us" className="py-10 sm:py-14 bg-muted/20 relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <AnimSection className="text-center max-w-3xl mx-auto mb-10">
              <Badge variant="outline" className="mb-4 text-primary border-primary/30 px-3 py-1.5">
              ...
              <p className="text-muted-foreground text-lg leading-relaxed">
                {get(
                  "why_subtitle",
                  "We're not just another tool — we're the only platform that truly unifies your online and offline business.",
                )}
              </p>
            </AnimSection>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-card rounded-2xl border border-border/50 overflow-hidden shadow-xl">
                <div className="grid grid-cols-3 bg-muted/50 border-b border-border/50">
                  <div className="p-4 sm:p-5 text-sm font-semibold text-muted-foreground">
                    {get("comparison_col_feature", "Feature")}
                  </div>
                  <div className="p-4 sm:p-5 text-center">
                    <div className="text-sm font-bold text-primary">{get("brand_name", "EvixPOS")}</div>
                  </div>
                  <div className="p-4 sm:p-5 text-center text-sm font-semibold text-muted-foreground">
                    {get("comparison_col_others", "Others")}
                  </div>
                </div>
                {get(
                  "comparison_features",
                  "Online + Offline in One Panel|Built-in POS System|Walk-in Sales Management|Multi-Store (Online & Offline)|Subscription & Renewal Tracking|WhatsApp Integration|Multi-Currency Support|Free Plan Available",
                )
                  .split("|")
                  .map((feature, i, arr) => (
                    <div
                      key={feature}
                      className={`grid grid-cols-3 ${i < arr.length - 1 ? "border-b border-border/30" : ""} hover:bg-muted/30 transition-colors`}
                    >
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              <AnimSection className="flex justify-center order-2 lg:order-1">
                <div className="relative">
                  ...
                  <motion.img
                    src={get("app_download_image") || mobileAppMockup}
                    alt="EvixPOS Mobile"
                    className="relative w-[260px] sm:w-[300px] drop-shadow-2xl"
                    loading="lazy"
              ...
              <AnimSection delay={0.15} className="order-1 lg:order-2">
                ...
                <p className="text-muted-foreground text-lg leading-relaxed mb-8">
                  {get(
                    ...
                    "Get the full power of EvixPOS on your mobile device. Available on all platforms.",
                  )}
                </p>
                <ul className="space-y-3 mb-8">
                  {get(
                    "mobile_features",
                    "Process walk-in POS sales from your phone|Track online orders in real-time|Manage inventory across all stores|View combined profit analytics",
                  )
                    .split("|")
                    .map((item) => (
                ...
                <div className="flex flex-wrap gap-3">
                  {get("app_download_android") ? (
                    <a href={get("app_download_android")} target="_blank" rel="noopener noreferrer">
                      <Button size="lg" className="gap-2 h-12 px-6">
                        <Download className="h-5 w-5" /> {get("mobile_btn_android", "Google Play")}
                      </Button>
                    </a>
                  ) : null}
                  {get("app_download_ios") ? (
                    <a href={get("app_download_ios")} target="_blank" rel="noopener noreferrer">
                      <Button size="lg" variant="outline" className="gap-2 h-12 px-6">
                        <Apple className="h-5 w-5" /> {get("mobile_btn_ios", "App Store")}
                      </Button>
                    </a>
                  ) : null}
                  {!get("app_download_android") && !get("app_download_ios") && (
                    <>
                      ...
                      >
                        ...
                        {canInstall ? get("mobile_btn_install", "Install App") : get("mobile_btn_web", "Use Web App")}
                      </Button>
                      ...
                      {isInstalled && (
                        <Button
                          size="lg"
                          variant="outline"
                          className="gap-2 h-12 px-6 text-primary border-primary/30"
                          disabled
                        >
        ...
        <section id="pricing" className="py-12 sm:py-16 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            <AnimSection className="text-center max-w-3xl mx-auto mb-8">
              ...
              <p className="text-muted-foreground text-lg leading-relaxed">
                {get("pricing_subtitle", "Start free. Upgrade when you're ready. No hidden fees.")}
              </p>
            </AnimSection>
              ...
              <div className="inline-flex bg-card rounded-xl border border-border/50 p-1 shadow-sm">
                {CURRENCIES.map((c) => (
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-all duration-300 ${currency === c.key ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    {c.symbol} {c.label}
                  </button>
                ))}
            ...
            <AnimSection delay={0.15} className="max-w-lg mx-auto mb-6">
              <div className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm p-5 shadow-sm">
                ...
                <div className="flex justify-between text-[10px] text-muted-foreground/60">
                  {get("pricing_volume_ticks", "500|1K|5K|10K|20K|50K|100K")
                    .split("|")
                    .map((l) => (
                      <span key={l}>{l}</span>
                    ))}
            ...
            <AnimSection delay={0.2} className="flex justify-center items-center gap-3 mb-10">
              <div className="inline-flex bg-card rounded-xl border border-border/50 p-1 shadow-sm">
                <button
                  ...
                  className={`px-5 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${!yearly ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {get("pricing_monthly_label", "Monthly")}
                </button>
                <button
                  onClick={() => setYearly(true)}
                  className={`px-5 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${yearly ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground hover:text-foreground"}`}
                >
                  {get("pricing_yearly_label", "Yearly")}
                </button>
              </div>
              {yearly && (
            ...
            {/* Plan Cards */}
            <div
              className={`grid md:grid-cols-2 gap-5 max-w-6xl mx-auto ${get("plan_custom_visible", "true") === "true" ? "lg:grid-cols-4" : "lg:grid-cols-3"}`}
            >
              {[
                {
                  name: get("plan_free_name", "Free"),
                  key: "free",
                  icon: Zap,
                  tagline: get("plan_free_tagline", "Forever free"),
                ...
                {
                  name: get("plan_pro_name", "Pro"),
                  key: "pro",
                  icon: Crown,
                  tagline: get("plan_pro_tagline", "Best for growing businesses"),
                  popular: true,
                  gradient: "from-primary/10 to-primary/5",
                  stores: "3",
                  products: "100",
                  customers: formatVolume(selectedVolume),
                  features: PLAN_FEATURES_LIST.pro,
                },
                {
                  name: get("plan_business_name", "Business"),
                  key: "business",
                  icon: Shield,
                  tagline: get("plan_business_tagline", "For scaling teams"),
                  bestValue: true,
                  gradient: "from-orange-500/10 to-orange-500/5",
                  stores: "10",
                  products: "500",
                  customers: formatVolume(Math.max(selectedVolume, 1000)),
                  features: PLAN_FEATURES_LIST.business,
                },
                ...(get("plan_custom_visible", "true") === "true"
                  ? [
                      {
                        name: get("plan_custom_name", "Custom"),
                        key: "custom",
                        icon: Star,
                        tagline: get("plan_custom_tagline", "For large businesses"),
                        gradient: "from-violet-500/10 to-violet-500/5",
                        stores: get("plan_unlimited_label", "Unlimited"),
                        products: get("plan_unlimited_label", "Unlimited"),
                        customers: get("plan_unlimited_label", "Unlimited"),
                        features: get(
                          "plan_custom_features",
                          "Everything in Business|Custom Integrations|Dedicated Account Manager|SLA Guarantee",
                        )
                          .split("|")
                          .filter(Boolean),
                      },
                ...
                return (
                  <AnimItem key={plan.key} delay={idx * 0.1}>
                    <Card
                      className={`relative overflow-hidden transition-all duration-500 hover:shadow-2xl group h-full ${
                        isPro
                          ? "border-primary/60 shadow-xl shadow-primary/10 ring-1 ring-primary/20"
                          : isBiz
                            ? "border-orange-400/40 shadow-lg"
                    ...
                    >
                      ...
                      {isBiz && (
                        <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-orange-500 via-orange-400 to-red-400" />
                      )}
                      ...
                      <CardContent className="pt-8 px-6 pb-6 relative flex flex-col h-full">
                        ...
                        <div className="flex items-center gap-3 text-[11px] text-muted-foreground mb-4 flex-wrap">
                          <span className="flex items-center gap-1">
                            <Store className="h-3 w-3" /> {plan.stores} {get("plan_unit_stores", "stores")}
                          </span>
                          <span className="flex items-center gap-1">
                            <Users className="h-3 w-3" /> {plan.customers}
                          </span>
                          <span className="flex items-center gap-1">
                            <Package className="h-3 w-3" /> {plan.products} {get("plan_unit_products", "products")}
                          </span>
                        </div>
                        ...
                        {plan.key === "free" ? (
                          <div className="mb-5">
                            ...
                            <span className="text-sm text-muted-foreground ml-1">
                              {get("plan_free_period", "/forever")}
                            </span>
                          </div>
                        ) : plan.key === "custom" ? (
                          <div className="mb-5">
                            <span className="text-3xl font-black text-violet-600">
                              {get("plan_custom_price_label", "Custom")}
                            </span>
                            <p className="text-xs text-muted-foreground mt-1">
                              {get("plan_custom_price_note", "Contact for pricing")}
                            </p>
                          </div>
                        ) : (
                          <div className="mb-5">
                            ...
                            <span className="text-sm text-muted-foreground">
                              {yearly ? get("plan_period_yearly", "/yr") : get("plan_period_monthly", "/mo")}
                            </span>
                            {yearly && (
                              <div className="mt-1">
                                <Badge variant="outline" className="text-emerald-600 border-emerald-500/30 text-[10px]">
                                  {get("pricing_yearly_off_badge", "20% OFF")}
                                </Badge>
                              </div>
                            )}
                            <p className="text-[11px] text-muted-foreground mt-1">
                              {yearly
                                ? get("pricing_validity_yearly", "Valid: 365 days")
                                : get("pricing_validity_monthly", "Valid: 30 days")}
                            </p>
                          </div>
                        )}
                        ...
                        >
                          {plan.key === "free"
                            ...
                            : plan.key === "custom"
                              ...
                              : plan.key === "pro"
                                ? loggedInUser
                                  ? get("plan_pro_cta_logged_in", "Upgrade Now")
                                  : get("plan_pro_cta", "Get Started")
                                : loggedInUser
                                  ? get("plan_business_cta_logged_in", "Upgrade Now")
                                  : get("plan_business_cta", "Get Started")}
      ...
      {show("testimonials") && (
        <section id="testimonials" className="py-10 sm:py-14 bg-muted/20 relative overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            <AnimSection className="text-center max-w-3xl mx-auto mb-10">
              <Badge variant="outline" className="mb-4 text-primary border-primary/30 px-3 py-1.5">
        ...
        <section id="faq" className="py-10 sm:py-14">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <AnimSection className="text-center mb-10">
              ...
              <p className="text-muted-foreground text-lg leading-relaxed">
                {get("faq_subtitle", "Everything you need to know before getting started.")}
              </p>
            </AnimSection>
            ...
            <AnimSection className="text-center mt-8">
              <p className="text-sm text-muted-foreground">
                {get("faq_help_text", "Still need help?")}{" "}
                <a
                  href={`mailto:${get("brand_email", "support@evixpos.com")}`}
                  className="text-primary font-medium hover:underline"
                >
                  {get("faq_help_cta", "Contact Support →")}
                </a>
              </p>
            </AnimSection>
          </div>
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative">
            <AnimSection>
              <motion.div
                ...
                className="w-20 h-20 rounded-3xl bg-gradient-to-br from-primary/30 to-primary/10 flex items-center justify-center mx-auto mb-8 shadow-2xl shadow-primary/30 border border-primary/20 overflow-hidden"
              >
                <img
                  src={get("brand_logo_mark") || brandLogoMark}
                  alt={get("brand_name", "EvixPOS")}
              ...
              <p className="text-muted-foreground text-lg sm:text-xl mb-10 max-w-2xl mx-auto leading-relaxed">
                {get(
                  "cta_subtitle",
                  "Join thousands of entrepreneurs who manage their entire business — online store, physical POS, inventory, customers — from one powerful platform.",
                )}
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Button
                  size="lg"
                  onClick={() => navigate("/auth")}
                  className="text-base px-10 h-12 gap-2 shadow-xl shadow-primary/25 hover:shadow-2xl transition-all"
                >
              ...
              <p className="text-sm text-muted-foreground mt-8 flex items-center justify-center gap-4 flex-wrap">
                <span className="flex items-center gap-1.5">
                  <Lock className="h-3.5 w-3.5" /> {get("cta_trust_1", "Secure & Private")}
                </span>
                <span className="flex items-center gap-1.5">
                  <CheckCircle2 className="h-3.5 w-3.5" /> {get("cta_trust_2", "No Credit Card")}
                </span>
                <span className="flex items-center gap-1.5">
                  <Clock className="h-3.5 w-3.5" /> {get("cta_trust_3", "Cancel Anytime")}
                </span>
              </p>
            </AnimSection>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-10 sm:py-16 grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-12 gap-x-4 gap-y-6 sm:gap-8 lg:gap-12">
            <div className="col-span-2 sm:col-span-2 lg:col-span-3">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed mb-6 max-w-xs">
                {get(
                  "footer_tagline",
                  "The all-in-one platform for online & offline business management — orders, POS, customers, inventory, and analytics in one panel.",
                )}
              </p>
              <a
                href={`mailto:${get("brand_email", "support@evixpos.com")}`}
                className="flex items-center gap-2.5 text-sm text-muted-foreground hover:text-primary transition-colors group mb-3"
              >
              ...
              <a
                href={`https://wa.me/${get("brand_whatsapp", "+91 8101949890").replace(/[\s+]/g, "")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2.5 text-sm text-muted-foreground hover:text-primary transition-colors group mb-5"
              >
                <div className="w-8 h-8 rounded-lg bg-primary/5 group-hover:bg-primary/10 flex items-center justify-center transition-colors">
              ...
              <div className="flex items-center gap-2">
                {[
                  ...
                  {
                    url: get("social_instagram", "https://instagram.com/evixpos"),
                    Icon: Instagram,
                    label: "Instagram",
                  },
            ...
            <div className="lg:col-span-2">
              <h4 className="font-semibold text-sm mb-2.5 sm:mb-4 text-foreground tracking-wide uppercase text-[11px]">
                {get("footer_col1_title", "Product")}
              </h4>
              <ul className="space-y-1.5 sm:space-y-3 text-sm text-muted-foreground">
                ...
                <li>
                  <button
                    ...
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_how", "How It Works")}
                  </button>
                </li>
                <li>
                  <button
                    onClick={() => smoothScroll("#screenshots")}
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_screenshots", "Screenshots")}
                  </button>
                </li>
                <li>
                  <button
                    onClick={() => smoothScroll("#integrations")}
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_integrations", "Integrations")}
                  </button>
                </li>
                <li>
                  <button
                    ...
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_updates", "What's New")}
                  </button>
                </li>
                <li>
                  <button onClick={() => navigate("/auth")} className="hover:text-primary transition-colors text-left">
                    {get("footer_link_demo", "Request Demo")}
                  </button>
                </li>
              </ul>
            </div>
            <div className="lg:col-span-2">
              ...
              <ul className="space-y-1.5 sm:space-y-3 text-sm text-muted-foreground">
                ...
                <li>
                  ...
                  >
                    {get("footer_link_contact", "Contact")}
                  </a>
                </li>
                <li>
                  <button
                    ...
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_privacy", "Privacy")}
                  </button>
                </li>
                <li>
                  <button
                ...
                <li>
                  <button
                    ...
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_refund", "Refund Policy")}
                  </button>
                </li>
              </ul>
            </div>
            <div className="lg:col-span-2">
              <h4 className="font-semibold text-sm mb-2.5 sm:mb-4 text-foreground tracking-wide uppercase text-[11px]">
                {get("footer_col3_title", "Account")}
              </h4>
              <ul className="space-y-1.5 sm:space-y-3 text-sm text-muted-foreground">
                <li>
                  <button onClick={() => navigate("/auth")} className="hover:text-primary transition-colors text-left">
                    {get("footer_link_login", "Login")}
                  </button>
                </li>
                <li>
                  <button onClick={() => navigate("/auth")} className="hover:text-primary transition-colors text-left">
                    {get("footer_link_signup", "Sign Up Free")}
                  </button>
                </li>
                <li>
                  <button
                    ...
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_dashboard", "Dashboard")}
                  </button>
                </li>
                <li>
                  <button
                    ...
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_billing", "My Plan")}
                  </button>
                </li>
                <li>
                  <button
                    ...
                    className="hover:text-primary transition-colors text-left"
                  >
                    {get("footer_link_support", "Support")}
                  </button>
                </li>
                <li>
                  <a
                    ...
                    className="hover:text-primary transition-colors"
                  >
                    {get("footer_link_whatsapp", "WhatsApp Us")}
                  </a>
                </li>
                <li>
                  <a
                    ...
                    className="hover:text-primary transition-colors"
                  >
                    {get("footer_link_email", "Email Us")}
                  </a>
                </li>
              </ul>
            </div>
            <div className="col-span-2 sm:col-span-2 lg:col-span-3">
              <h4 className="font-semibold text-sm mb-2.5 sm:mb-4 text-foreground tracking-wide uppercase text-[11px]">
                {get("footer_why_title", "Why EvixPOS")}
              </h4>
              <ul className="space-y-2.5 mb-6">
                {get(
                  "footer_highlights",
                  "Online + Offline in One Panel|Multi-Store Support|24/7 Support|Fast & Reliable",
                )
                  .split("|")
                  .map((item, idx) => (
              ...
              <div className="p-4 rounded-xl bg-primary/5 border border-primary/10">
                ...
                <Button
                  size="sm"
                  onClick={() => navigate("/auth")}
                  className="gap-1.5 shadow-md shadow-primary/15 w-full"
                >
                  {get("footer_cta_button", "Get Started")} <ArrowRight className="h-3.5 w-3.5" />
                </Button>
              </div>
            </div>
          </div>
          <div className="border-t border-border/40 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-xs text-muted-foreground">
              {get("footer_copyright", `© 2026 EvixPOS. All rights reserved.`)}{" "}
              {get("footer_powered_prefix", "Powered by")}{" "}
              <a
                href={get("footer_powered_url", "https://www.lifeaimit.in")}
                target="_blank"
                rel="noopener noreferrer"
                className="text-primary hover:underline font-medium"
              >
                {get("footer_powered_by", "LifeAim IT")}
              </a>
            </p>
            <div className="flex items-center gap-6 text-xs text-muted-foreground">
              <button onClick={() => setPolicyModal("privacy")} className="hover:text-primary transition-colors">
                {get("footer_privacy_label", "Privacy Policy")}
              </button>
              <button onClick={() => setPolicyModal("terms")} className="hover:text-primary transition-colors">
                {get("footer_terms_label", "Terms of Service")}
              </button>
              <button onClick={() => setPolicyModal("refund")} className="hover:text-primary transition-colors">
                {get("footer_refund_label", "Refund Policy")}
              </button>
            </div>
          </div>
        </div>
      ...
      <AnimatePresence>
        {scrolled && (
          <motion.div
            ...
            animate={{ y: 0 }}
            exit={{ y: 100 }}
            className="fixed bottom-0 inset-x-0 z-40 sm:hidden bg-card/90 backdrop-blur-xl border-t border-border/50 px-4 py-3 safe-area-pb"
          >
      ...
      {get("hero_video_url") && (
        <VideoModal
          ...
          videoType={get("hero_video_type", "youtube") as "youtube" | "mp4"}
          thumbnail={get("hero_video_thumbnail") || undefined}
          title={get("hero_video_title", "EvixPOS Product Demo")}
        />
      )}
      <LandingChatbot />
      <ScrollToTopButton />
