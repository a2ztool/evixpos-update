      ...
      <section className="relative pt-32 sm:pt-36 lg:pt-40 pb-10 sm:pb-14 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex justify-center mb-6 sm:mb-8">
            <div className="relative rounded-full p-[1.5px] overflow-hidden max-w-[95vw] group">
              ...
              <span
                ...
                style={{
                  background:
                    "conic-gradient(from var(--social-angle, 0deg), hsl(var(--primary)) 0%, hsl(var(--primary)/0.15) 25%, hsl(var(--primary)) 50%, hsl(var(--primary)/0.15) 75%, hsl(var(--primary)) 100%)",
                  animation: "spin-slow 4s linear infinite",
                }}
              />
              <div className="relative inline-flex items-center gap-1 sm:gap-3 px-2 sm:px-5 rounded-full bg-card/95 backdrop-blur-sm shadow-sm h-5 sm:h-8">
                <div className="flex -space-x-1 sm:-space-x-2 shrink-0">
                  {[1,2,3].map(i => {
                    const img = get(`social_proof_avatar_${i}`);
                    return img ? (
                      <img key={i} src={img} className="w-3 h-3 sm:w-6 sm:h-6 rounded-full border-2 border-card object-cover" alt="" />
                    ) : (
                      <div key={i} className="w-3 h-3 sm:w-6 sm:h-6 rounded-full border-2 border-card bg-gradient-to-br from-primary/40 to-primary/20 flex items-center justify-center">
                        <Users className="h-1.5 w-1.5 sm:h-3 sm:w-3 text-primary-foreground" />
                      </div>
                    );
                  })}
                </div>
                <span className="text-[9px] sm:text-[13px] font-medium text-muted-foreground line-clamp-1 leading-none">
                  {get("hero_social_proof", "Trusted by 3,000+ businesses worldwide")}
                </span>
                <Button variant="ghost" size="sm" onClick={() => navigate("/auth")} className="!h-4 sm:!h-6 px-1 sm:px-2.5 text-[9px] sm:text-xs font-semibold text-primary hover:text-primary shrink-0 leading-none min-h-0">
