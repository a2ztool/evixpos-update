        ...
        {bannerActive && (
          <div className={`bg-gradient-to-r from-primary via-primary/90 to-primary/80 text-primary-foreground text-center text-[9px] sm:text-sm font-medium relative overflow-hidden transition-all duration-300 ${scrolled ? 'max-h-0 opacity-0' : 'h-6 sm:max-h-12 opacity-100'}`}>
            ...
            <div className="max-w-7xl mx-auto px-2 sm:px-4 h-6 sm:h-auto flex items-center justify-center gap-1 sm:gap-3 relative leading-none">
              ...
              <Button size="sm" variant="secondary" className="!h-[16px] sm:!h-7 text-[8px] sm:text-xs px-1.5 sm:px-3 py-0 leading-none rounded shrink-0 min-h-0" onClick={() => navigate("/auth")}>
          ...
          >
            ...
            <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
              ...
              <Button 
                size="sm" 
                onClick={() => navigate("/auth")} 
                className="gap-1 sm:gap-1.5 text-[10px] sm:text-[13px] font-semibold !h-5 sm:!h-10 sm:px-5 rounded-full bg-primary hover:bg-primary/90 shadow-[0_4px_16px_-4px_hsl(var(--primary)/0.4)] hover:shadow-[0_6px_20px_-4px_hsl(var(--primary)/0.5)] active:scale-[0.96] transition-all duration-200 py-0 px-2 leading-none min-h-0"
              >
      ...
      <section className="relative pt-32 sm:pt-36 lg:pt-40 pb-10 sm:pb-14 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex justify-center mb-6 sm:mb-8">
            <div className="inline-flex items-center gap-1 sm:gap-3 px-2 sm:px-5 rounded-full bg-card/80 backdrop-blur-sm border border-border/50 shadow-sm max-w-[95vw] h-5 sm:h-auto">
              <div className="flex -space-x-1 sm:-space-x-2 shrink-0">
                {[1,2,3].map(i => {
                  const img = get(`social_proof_avatar_${i}`);
                  return img ? (
                    <img key={i} src={img} className="w-3 h-3 sm:w-7 sm:h-7 rounded-full border-2 border-card object-cover" alt="" />
                  ) : (
                    <div key={i} className="w-3 h-3 sm:w-7 sm:h-7 rounded-full border-2 border-card bg-gradient-to-br from-primary/40 to-primary/20 flex items-center justify-center">
                      <Users className="h-1.5 w-1.5 sm:h-3 sm:w-3 text-primary-foreground" />
                    </div>
                  );
                })}
              </div>
              <span className="text-[9px] sm:text-sm font-medium text-muted-foreground line-clamp-1 leading-none">
                {get("hero_social_proof", "Trusted by 3,000+ businesses worldwide")}
              </span>
              <Button variant="ghost" size="sm" onClick={() => navigate("/auth")} className="!h-4 sm:!h-7 px-1 sm:px-3 text-[9px] sm:text-xs font-semibold text-primary hover:text-primary shrink-0 leading-none min-h-0">
