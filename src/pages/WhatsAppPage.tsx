        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setGuideOpen(!guideOpen)}>
              <BookOpen className="h-4 w-4 mr-1.5" />{guideOpen ? "Guide বন্ধ করুন" : "Setup Guide"}
            </Button>
            <Button variant="outline" size="sm" onClick={() => setBulkOpen(true)} disabled={!wa || wa.status !== "active"}>
              <Users className="h-4 w-4 mr-1.5" />Bulk Send
            </Button>
            <Button size="sm" onClick={() => setSendOpen(true)} disabled={!wa || wa.status !== "active"}>
          ...
          <Card className="border-primary/20 bg-primary/5">
            <CardHeader className="pb-3">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg">📖 WhatsApp Business API সেটআপ গাইড</CardTitle>
              </div>
              <p className="text-sm text-muted-foreground">ধাপে ধাপে নিচের নির্দেশনা অনুসরণ করুন। প্রতিটি ধাপ ক্লিক করে বিস্তারিত দেখুন।</p>
            </CardHeader>
            <CardContent className="space-y-2">
              {setupSteps.map((step) => (
                <Collapsible key={step.num} open={openSteps.includes(step.num)}>
                  ...
                  <CollapsibleContent className="pl-14 pr-4 pb-3">
                    {step.content}
                  </CollapsibleContent>
                </Collapsible>
              ))}
          ...
          <TabsContent value="dashboard" className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {[
                ...
                { label: "Successful", value: stats.sent, icon: CheckCircle2, color: "text-green-500" },
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={() => setSendOpen(true)} disabled={!wa || wa.status !== "active"}>
                    <Send className="h-5 w-5 text-green-500" />
                    <span className="text-xs">Send Message</span>
                  </Button>
                  <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={() => setBulkOpen(true)} disabled={!wa || wa.status !== "active"}>
                    <Users className="h-5 w-5 text-blue-500" />
                    <span className="text-xs">Bulk Send</span>
                  </Button>
                  <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={testConnection} disabled={!wa || testing}>
                    <Zap className="h-5 w-5 text-yellow-500" />
                    <span className="text-xs">{testing ? "Testing..." : "Test Connection"}</span>
                  </Button>
                  <Button variant="outline" className="h-auto py-4 flex-col gap-2" onClick={fetchData}>
            ...
            <Card>
              <CardHeader className="flex-row items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => setActiveTab("logs")}>View All →</Button>
              </CardHeader>
              <CardContent>
                {logs.length === 0 ? (
                  <p className="text-muted-foreground text-center py-8 text-sm">কোনো মেসেজ পাঠানো হয়নি এখনও।</p>
                ) : (
                    ...
                    {logs.slice(0, 5).map((l) => (
                      <div key={l.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium">{l.recipient}</p>
