...
// Triggered by pg_cron via http_post; can also be invoked manually.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Compute target dates (today + 7, +3, +1) at UTC date level
    ...
    // Group active WA integrations cache by user_id+store_id
    const integrationCache = new Map<string, any>();

    for (const t of targets) {
      // Subscriptions expiring exactly on this target date
      ...
      if (subErr) {
        errors.push(`Fetch failed for ${t.days}d: ${subErr.message}`);
        continue;
      }
      ...
      for (const s of subs as any[]) {
        ...
        // Dedupe: skip if already reminded for this sub at this days bucket today
        const { data: already } = await supabase
          .from("subscription_reminders")
          .select("id")
          .eq("subscription_id", s.id)
          .eq("days_before", t.days)
          .eq("sent_date", today.toISOString().slice(0, 10))
          .maybeSingle();
        if (already) { skipped++; continue; }

        // Get WhatsApp integration for this user+store
        ...
        try {
          ...
          // Log reminder + create in-app notification for owner
          ...
          await supabase.from("notifications").insert({
            user_id: s.user_id,
            type: "info",
            message: `📲 WhatsApp renewal reminder sent to ${name || phone} for "${s.product_name}" (${t.days}d left).`,
          });
